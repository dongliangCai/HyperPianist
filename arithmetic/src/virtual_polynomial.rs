...
use crate::{
    eq_poly::EqPolynomial, errors::ArithErrors, multilinear_polynomial::random_zero_mle_list,
    random_mle_list, unsafe_allocate_zero_vec,
};
...
impl<F: PrimeField> VirtualPolynomial<F> {
    ...
    pub fn mul_by_mle(
        ...
        if self.flattened_ml_extensions.len() != 0
            && mle.num_vars != self.flattened_ml_extensions[0].num_vars
        {
            return Err(ArithErrors::InvalidParameters(format!(
                "product has a multiplicand with wrong number of variables {} vs {}",
                mle.num_vars, self.flattened_ml_extensions[0].num_vars
    ...
    /// Sample a random virtual polynomial, return the polynomial and its sum.
    pub fn rand_fixed_coeff<R: RngCore>(
        nv: usize,
        num_multiplicands_range: (usize, usize),
        num_products: usize,
        rng: &mut R,
    ...
    /// Sample a random virtual polynomial that evaluates to zero everywhere
    /// over the boolean hypercube.
    pub fn rand_zero_fixed_coeff<R: RngCore>(
        nv: usize,
        num_multiplicands_range: (usize, usize),
        num_products: usize,
        rng: &mut R,
    ...
    pub fn build_f_hat_with_coeff(&self, r: &[F], coeff: &F) -> Result<Self, ArithErrors> {
        let start = start_timer!(|| "zero check build hat f");
        ...
        let eq_x_r = build_eq_x_r_with_coeff(r, coeff)?;
        let mut res = self.clone();
        res.mul_by_mle(eq_x_r, F::one())?;
...
pub fn build_eq_x_r_vec<F: PrimeField>(r: &[F]) -> Result<Vec<F>, ArithErrors> {
    ...
    let r_rev = r.iter().rev().cloned().collect::<Vec<_>>();
    let eval = EqPolynomial::evals(&r_rev);
...
pub fn build_eq_x_r_with_coeff<F: PrimeField>(
    ...
    let evals = build_eq_x_r_vec_with_coeff(r, coeff)?;
    let mle = DenseMultilinearExtension::from_evaluations_vec(r.len(), evals);
...
/// over r, which is
///      eq(x,y) = \prod_i=1^num_var (x_i * r_i + (1-x_i)*(1-r_i))
pub fn build_eq_x_r_vec_with_coeff<F: PrimeField>(
    r: &[F],
    coeff: &F,
    ...
    let r_rev = r.iter().rev().cloned().collect::<Vec<_>>();
    let eval = EqPolynomial::evals_coeff(&r_rev, coeff);
...
/// This function takes `r.len()` steps, and for each step it requires a maximum
/// `r.len()-1` multiplications.
fn build_eq_x_r_helper_with_coeff<F: PrimeField>(
    r: &[F],
    coeff: &F,
    buf: &mut Vec<F>,
) -> Result<(), ArithErrors> {
    if r.is_empty() {
        ...
        build_eq_x_r_helper_with_coeff(&r[1..], coeff, buf)?;
...
mod test {
    ...
    fn test_eq_xr_coeff() {
        ...
        for nv in 4..10 {
            let r: Vec<Fr> = (0..nv).map(|_| Fr::rand(&mut rng)).collect();
            let mut eq_x_r = build_eq_x_r_vec(r.as_ref()).unwrap();

            let coeff = Fr::rand(&mut rng);
            let eq_x_r2 = build_eq_x_r_vec_with_coeff(r.as_ref(), &coeff).unwrap();
            eq_x_r.iter_mut().for_each(|val| *val *= coeff);
    ...
    fn test_build_eq_table() {
        ...
        for nv in 4..10 {
            let r: Vec<Fr> = (0..nv).map(|_| Fr::rand(&mut rng)).collect();
            let table = build_eq_table(&r, Fr::one());
            for k in 1..r.len() {
                assert_eq!(table[k - 1], build_eq_x_r_vec(&r[k..]).unwrap());
            }
        }
    }
}
